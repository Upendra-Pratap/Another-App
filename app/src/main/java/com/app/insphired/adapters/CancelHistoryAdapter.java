...
import com.app.insphired.apimodel.CancelEmployeeHistoryList;
...
public class CancelHistoryAdapter extends RecyclerView.Adapter<CancelHistoryAdapter.ViewHolder> {
    ...
    public void onBindViewHolder(CancelHistoryAdapter.ViewHolder holder, @SuppressLint("RecyclerView") int position) {
        ...
        String userTypeeC = (cancelEmployeeHistoryList.get(position).getUserType());
        if (status.equals("3"))
        {
        ...
        else if  (userTypeeC.equals("2"))
        {
            Glide.with(context).load(Api_Client.BASE_URL_IMAGES2 + cancelEmployeeHistoryList.get(position).getImage()).into(holder.imgCancel);
        }
