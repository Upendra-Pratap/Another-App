...
import com.app.insphired.apimodel.EmployeeHistoryCompleteList;
import com.app.insphired.EmployeeActivity.EmployeeGiveRatingActivity;
...
public class EmployeeHistoryAdapter extends RecyclerView.Adapter<EmployeeHistoryAdapter.ViewHolder> {
    ...
    @Override
    public void onBindViewHolder(@NonNull EmployeeHistoryAdapter.ViewHolder holder, @SuppressLint("RecyclerView") int position) {
      ...
      String userTypee = (employeeHistoryCompleteLists.get(position).getUserType());
      if (userTypee.equals("1"))
      {
      ...
      else if  (userTypee.equals("2"))
      {
          Glide.with(context).load(Api_Client.BASE_URL_IMAGES2 + employeeHistoryCompleteLists.get(position).getImage()).into(holder.imgEEH);
      }
